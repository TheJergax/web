...
function initializeFirebase() {
    const firebaseConfig = {
        apiKey: "AIzaSyCJgzLZC1ixHKkpFIvYIhUJuUL5EcGxWak",
        authDomain: "residential-management-app.firebaseapp.com",
        projectId: "residential-management-app",
        storageBucket: "residential-management-app.appspot.com",
...
function loadData() {
    ...
    Promise.all([
        ...
        db.collection('residentialUnits').get(),
        db.collection('activeVisitors').get(),
        db.collection('visitorsHistory').get(),
        db.collection('activePackages').get(),
        db.collection('packagesHistory').get(),
        db.collection('residents').get()
    ]).then(([
        usersSnapshot, 
        unitsSnapshot, 
        activeVisitorsSnapshot, 
        visitorsHistorySnapshot, 
        activePackagesSnapshot, 
        packagesHistorySnapshot, 
        residentsSnapshot
    ]) => {
        ...
        const adminExists = loadedUsers.some(user => user.username === 'administrador');
        users = adminExists ? loadedUsers : [...loadedUsers, ...users];
        
        residentialUnits = unitsSnapshot.docs.map(doc => ({id: doc.id, ...doc.data()}));
        activeVisitors = activeVisitorsSnapshot.docs.map(doc => ({id: doc.id, ...doc.data()}));
        visitorsHistory = visitorsHistorySnapshot.docs.map(doc => ({id: doc.id, ...doc.data()}));
        activePackages = activePackagesSnapshot.docs.map(doc => ({id: doc.id, ...doc.data()}));
        packagesHistory = packagesHistorySnapshot.docs.map(doc => ({id: doc.id, ...doc.data()}));
        residents = residentsSnapshot.docs.map(doc => ({id: doc.id, ...doc.data()}));
...
function saveToFirebase() {
    ...
    // Clear existing collections and add new data
    return clearAndSetCollection(db, 'users', users, batch)
        .then(() => clearAndSetCollection(db, 'residentialUnits', residentialUnits, batch))
        .then(() => clearAndSetCollection(db, 'activeVisitors', activeVisitors, batch))
        .then(() => clearAndSetCollection(db, 'visitorsHistory', visitorsHistory, batch))
        .then(() => clearAndSetCollection(db, 'activePackages', activePackages, batch))
        .then(() => clearAndSetCollection(db, 'packagesHistory', packagesHistory, batch))
        .then(() => clearAndSetCollection(db, 'residents', residents, batch))
        .then(() => batch.commit());
}

function clearAndSetCollection(db, collectionName, data, batch) {
